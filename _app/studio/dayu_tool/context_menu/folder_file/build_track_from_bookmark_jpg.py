...
class BuildBookmark(MActionBase):
    def __init__(self, parent=None):
        ...
        self.name = self.tr('DaYu Build Track JPG')
    ...
    def run(self, event, **kwargs):
        ...
        if event.selection and len(event.selection) >= 1:
            ...
            with current_project.beginUndo('build new track'):
                ...
                for version_orm in version_list:
                    ...
                    if version_orm.type_group_name == 'dailies':
                        sub_file = next((x for x in version_orm.sub_level.walk(collapse=False, relative=True) if 'jpg/' in x), None)
                        ...
                        binitem = h_read_a0001.create(seq_file, version_orm, 'jpg')
