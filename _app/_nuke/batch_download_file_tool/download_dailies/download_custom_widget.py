...
from _app._public.context_menu.folder_file.download_dailies.attr_config import COLOR_SPACE_CONFIG, META_CODEC_DICT
...
class DownloadDailiesDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(self.tr('Download Custom Dailies'))
        self.project = None
        geo = QApplication.desktop().screenGeometry()
        self.setGeometry(geo.width() / 4, geo.height() / 4, geo.width() / 2, geo.height() / 2)
        self.title_label = QLabel(self.tr('Download Custom Dailies'))
        ...
        self.folder_rename_check_box = QCheckBox(self.tr('Folder Rename:'))
        self.rename_check_box.setChecked(True)
        self.folder_rename_line_edit = QLineEdit()
        self.folder_rename_check_box.stateChanged.connect(self.folder_rename_line_edit.setEnabled)
        self.folder_rename_check_box.setChecked(False)
    ...
    def setup_data(self, version_orm_list):
        ...
        for orm in version_orm_list:
            ...
            for sub_level in orm.sub_level.walk(collapse=True):
                if '/cloud/' in sub_level.filename.replace(orig_folder, ''):
                    continue
                result_list.append({
                        'name': orm.name,
                        'name_checked': 2,
        ...
        setattr(self, 'orig_data', result_list)
        self.slot_ext_changed(self.ext_com_box.currentText())
    ...
    @staticmethod
    def get_like_lut_orm(orm, shot):
        folder_table = db.util.get_class('folder')
        resource_orm_list = db.get_session().query(folder_table) \
            .filter(folder_table.meaning.like('%SHOT_ELEMENT_RESOURCE')) \
            .filter(folder_table.name.like('{}_lut'.format(shot.name) + '%')) \
            .filter(folder_table.top == orm.top) \
            .all()
        if resource_orm_list:
            orm = list(resource_orm_list[-1].children)[-1]
            return orm
    ...
    def slot_download(self):
        ...
        for data_obj in self.item_view.get_data():
            ...
            if self.lut_check_box.isChecked():
                lut_orm = self.get_like_lut_orm(orm, orm.find_meaning('SHOT'))
                lut_file = lut_orm.flatten()[-1].get('file') if lut_orm and lut_orm.flatten() else None
                lut_filename = lut_file % lut_file.frames[0] if lut_file else ''
            else:
                lut_filename = ''
            meta_codec_type = META_CODEC_DICT[self.codec_com_box.currentText()] \
                if self.codec_check_box.isChecked() else 'apco'
            ...
            if self.use_sg_client_name_check_box.isChecked():
                ...
                project_ = shot.get("project")["name"]
                orm.comment = shot.get('sg_client_name')
            ...
            if project_ == "mzh":
                if orm.comment:
                    new_name = orm.comment
                else:
                    new_name = new_name.split("_")[0] + "_" + new_name.split("_")[1]
            if project_ == "fc2":
                new_name = self._resolver.resolve(orm, self.rename_line_edit.text())

            if self.use_time_check_box.isChecked():
                orm.comment = time.strftime("%Y%m%d", time.localtime())

            if self.folder_rename_check_box.isChecked():
                folder_name = self._resolver.resolve(orm, self.folder_rename_line_edit.text())
            else:
                folder_name = new_name

            if self.sequence_check_box.isChecked():
                sequence_name = new_name.split("_")[0]
                out_name = os.path.join(str(target_obj), sequence_name, new_name + ".mov")
                out_path = out_name.replace("\\", "/")
            else:
                out_path = DayuPath(target_obj).child(folder_name, '{}.mov'.format(new_name))
            ...
            version_dict.setdefault(orm.name, {
                ...
                'cilent_name': orm.comment,
                'meta_codec_type': meta_codec_type,
                'out_path': str(out_path),
                'fps': fps,
                'time_code': orm.extra_data.get('time_code') if orm.extra_data else '',
                'extra_data': '{}*{}'.format(orm.name, orm.find_meaning('SHOT').cloud_id),
                'comment': comment,
                'updated_by_name': orm.find_meaning('SHOT').updated_by_name
            })
    ...
    def nuke_render(self, version_dict):
        ...
        """
        template_py = DayuPath(__file__).parent.child('template_script.py')
        ...
        scripts_path = DayuPath(os.path.expanduser('~')).child('download_custom_dailies.py')
    ...
    def _write_to_csv(self, csv_data_dict):
        for key, data_list in csv_data_dict.items():
            file_path = os.path.join(self.folder_widget.get_folder(),
                                     '{}-{}.csv'.format(dt.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'), key))
