...
class OptionsContainer(QWidget):
    def __init__(self, main_window):
        ...
        self.capture_name = QLineEdit()
        self.capture_name.setText("capture")
        self.layout.addWidget(QLabel("Capture Name:"), 40, 10)
        self.layout.addWidget(self.capture_name, 50, 10, 1, 10)

        self.encode_bt = QPushButton("Encode Gif")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.connect(self.options.encode_bt, SIGNAL("clicked()"), self.encode_gif)
    ...
    def capture(self):
        ...
        if not self.snap_shots.queue.empty():
            ...
            dir = self.options.capture_name.text()
            file_postfix = "{0:03}".format(self.capture_count)
            self.px2.save((dir + '/screenshot_%s.png' % (file_postfix)), 'png')
    ...
    def encode_gif(self):
        ...
        for f in os.listdir(dir):
            m = re.search(rex, f)
            if m:
                frames.append((int(m.group(1)), dir + "/" + f))
            frames.sort()
        ...
        writeGif(dir + '.gif', images, duration=0.5, dither=0)
