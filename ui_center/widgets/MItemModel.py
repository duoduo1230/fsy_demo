...
from ui_utils import default_formatter, apply_formatter, get_font, icon_formatter, \
    get_obj_value, set_obj_value, has_obj_value

SETTING_MAP = {
    Qt.BackgroundRole: {'key': 'bg_color', 'formatter': QColor},
    Qt.DisplayRole: {'key': 'display', 'formatter': default_formatter},
    Qt.EditRole: {'key': 'edit', 'formatter': None},
    Qt.TextAlignmentRole: {'key': 'alignment',
                           'formatter': {'right': Qt.AlignRight, 'left': Qt.AlignLeft, 'center': Qt.AlignCenter}},
    Qt.ForegroundRole: {'key': 'color', 'formatter': QColor},
    Qt.FontRole: {'key': 'is_link', 'formatter': get_font},
    Qt.DecorationRole: {'key': 'icon', 'formatter': icon_formatter},
    Qt.ToolTipRole: {'key': 'tooltip', 'formatter': default_formatter},
    Qt.InitialSortOrderRole: {'key': 'order', 'formatter': {'asc': Qt.AscendingOrder, 'des': Qt.DescendingOrder}},
    Qt.SizeHintRole: {'key': 'size', 'formatter': lambda args: QSize(*args)},
    Qt.UserRole: {'key': 'data'},  # anything
}
...
class MTableModel(QAbstractItemModel):
    ...
    def flags(self, index):
        ...
        elif self.header_list[index.column()].get('checkable', False):
            return Qt.ItemFlags(QAbstractItemModel.flags(self, index) | Qt.ItemIsUserCheckable)
        elif self.header_list[index.column()].get('draggable', False):
            return Qt.ItemFlags(QAbstractItemModel.flags(self, index) | Qt.ItemIsDragEnabled | Qt.ItemIsDropEnabled)
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        attr = attr_dict.get('attr')
        if role in SETTING_MAP.keys():
            role_key = SETTING_MAP[role].get('key')  # role 配置的关键字
