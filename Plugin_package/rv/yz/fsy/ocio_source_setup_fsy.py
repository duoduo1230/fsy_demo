...
default_config_1 = "D:/temp/rv/YZ/OpenColorIO-Config-ACES-1.2/aces_1.2/config.ocio"
default_config = "D:/temp/rv/YZ/aces_1.0.3/config_new.ocio"
...
def is_sRGB(media):
    filename = os.path.basename(media)
    isTure = (filename.find("Utility - sRGB") != -1) or os.path.splitext(media)[1] in [".avi", ".jpg", ".jpeg", ".mov",".png"]
    return media is not None and isTure
    # return media is not None and  os.path.splitext(media)[1] in [".avi", ".jpg", ".jpeg", ".mov",".png"]
...
def ocio_node_from_media(config, node, default, media=None, attributes={}):
    ...
    if is_media_raw:
        ...
        print(
            ("ERROR: working_space %s" % working_space)
        )
    # elif is_media_srgb:
    ...
    if is_media_raw:
        ...
        print(
            ("ERROR: is media raw sourceColorSpace %s" % sourceColorSpace)
        )
        
    elif is_media_srgb:
        sourceColorSpace = "Output - Rec.709"
        print(
            ("ERROR: is media SRGB sourceColorSpace %s" % sourceColorSpace)
        )
    else:
        sourceColorSpace = config.getColorSpace(OCIO.Constants.ROLE_SCENE_LINEAR).getName()
        print(
            ("ERROR: %s" % sourceColorSpace)
        )

    if nodeType == "RVDisplayPipelineGroup":
        print("nodeType:%s" % "RVDisplayPipelineGroup")
        ...
        # if inspace != "":
        print("nodeType:%s" % "RVLinearizePipelineGroup")
        result = [
            {
                "nodeType": "OCIOFile",
                "context": {},
        ...
        print("nodeType:%s" % "RVLookPipelineGroup")
        result = [{
            "nodeType": "OCIOLook",
            "context": {},
            "properties": {
...
class OCIOSourceSetupMode(rvtypes.MinorMode):
    ...
    def useSourceOCIO(self, source, nodeType, defaultSetting=""):
        ...
        try:
            ...
            print(
                ("ERROR: Unable to make use of ocio_node_from_media_ return: %s" % inst)
            )
    ...
    def buildOCIOMenu(self):
        ...
        if self.config == None:
            try:
                ...
                OCIO.SetCurrentConfigSetCurrentConfig(self.config)
            except Exception as inst:
