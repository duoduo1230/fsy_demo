...
def batch_retarget(character, source, animation_fbx, output, fps=60):
    ...
    try:
        ...
        # 导入source绑定的max动画文件并bake control rig
        ...
        humanIK_api.bake_to_control_rig()
        ...
        cmds.file(rename='{}/us_{}.ma'.format(output, anim_path.stem))
        cmds.file(force=True, type='mayaAscii', save=True)

        # bake骨骼并导出FBX
        fbx_path = '{}/us_{}.fbx'.format(output, anim_path.stem)
        joints = cmds.ls("{}:root".format(namespace), dag=True, type="joint")
